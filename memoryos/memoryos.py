import os
import json
from .utils import OpenAIClient, get_timestamp, generate_id, gpt_personality_analysis, gpt_update_profile, ensure_directory_exists
...
H_PROFILE_UPDATE_THRESHOLD = 2.0 
    ...
    def _trigger_profile_and_knowledge_update_if_needed(self):
        ...
        if current_heat >= self.mid_term_heat_threshold:
            ...
            if unanalyzed_pages:
                ...
                # Perform personality and knowledge analysis using these pages
                # The gpt_personality_analysis function in utils.py now handles assistant knowledge too
                analysis_result = gpt_personality_analysis(unanalyzed_pages, self.client, model=self.llm_model)
                
                new_user_profile_text = analysis_result.get("profile")
                new_user_private_knowledge = analysis_result.get("private")
                new_assistant_knowledge = analysis_result.get("assistant_knowledge")
                ...
                if new_user_profile_text and new_user_profile_text.lower() != "none":
                    ...
                    self.user_long_term_memory.update_user_profile(self.user_id, updated_profile)
