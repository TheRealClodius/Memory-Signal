...
def gpt_personality_analysis(dialogs, client: OpenAIClient, model="gpt-4o-mini", known_user_traits="None"):
    ...
    print("Calling LLM for personality analysis...")
    ...
    try:
        if "【User Profile】" in result_text:
            profile_start = result_text.find("【User Profile】") + len("【User Profile】")
            if "【User Private Data】" in result_text:
                profile_end = result_text.find("【User Private Data】")
                profile = result_text[profile_start:profile_end].strip()
                ...
                if "【Assistant Knowledge】" in result_text:
                    ...
                    private_data = result_text[private_data_start:].strip() # Assistant knowledge might be missing
            else: # No private data or assistant knowledge
                profile = result_text[profile_start:].strip()
        elif "【User Private Data】" in result_text: # Only private data and potentially assistant knowledge
            private_data_start = result_text.find("【User Private Data】") + len("【User Private Data】")
            if "【Assistant Knowledge】" in result_text:
                private_data_end = result_text.find("【Assistant Knowledge】")
                private_data = result_text[private_data_start:private_data_end].strip()
        ...
        elif "【Assistant Knowledge】" in result_text: # Only assistant knowledge
             assistant_knowledge_start = result_text.find("【Assistant Knowledge】") + len("【Assistant Knowledge】")
             assistant_knowledge = result_text[assistant_knowledge_start:].strip()

    except Exception as e:
        print(f"Error parsing personality analysis: {e}. Raw result: {result_text}")
