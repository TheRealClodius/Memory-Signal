...
SUMMARIZE_DIALOGS_SYSTEM_PROMPT = "You are an expert in summarizing dialogue topics, please generate a concise and precise summary."
SUMMARIZE_DIALOGS_USER_PROMPT = "Please generate a topic summary based on the following conversation：\n{dialog_text}\nSubject Summary："

# Prompt for multi-summary generation (from utils.py, gpt_generate_multi_summary)
MULTI_SUMMARY_SYSTEM_PROMPT = "You are an expert in analyzing dialogue topics. No more than two topics."
MULTI_SUMMARY_USER_PROMPT = ("Please analyze the following dialogue and generate multiple subtopic summaries (if applicable), with a maximum of two themes.\n"
...
PERSONALITY_ANALYSIS_SYSTEM_PROMPT = """You are a professional user preference analysis assistant. Your task is to analyze the user's personality preferences from the given dialogue based on the provided dimensions, and also extract user private data and assistant knowledge.
...
Focus on the user's preferences and traits for the personality analysis section.
Also extract any private user data and assistant knowledge demonstrated in the conversation.
Output all three sections: User Profile, User Private Data, and Assistant Knowledge.
"""
...
【User Profile】
(Begin your analysis here, listing each reflected dimension as per the format above)

【User Private Data】
Please extract any personal information, preferences, or private facts about the user mentioned in the conversation:
- [Fact 1]: [Details with context]
- [Fact 2]: [Details with context]
- (If no private data found, write "None")

【Assistant Knowledge】
Please extract any explicit statements about what the assistant did, provided, or demonstrated in this conversation. Use the format "Assistant [action/capability] at [time/context]":
- Assistant [specific action/knowledge demonstrated] at [timestamp/context]
- Assistant [specific capability shown] during [specific interaction]
- (If no assistant knowledge found, write "None")

Examples:
- Assistant recommended Interstellar movie on 2023-10-01
- Assistant provided cooking recipe for pasta during dinner discussion
- Assistant demonstrated knowledge of Python programming when helping with code
- Assistant showed ability to analyze data during spreadsheet discussion
"""
...
EXTRACT_KEYWORDS_SYSTEM_PROMPT = "You are an expert in keyword extraction. Extract the most relevant keywords from the text. Return keywords as a comma-separated list."
EXTRACT_KEYWORDS_USER_PROMPT = "Please extract keywords from the following text:\n{text}\n\nKeywords:"
